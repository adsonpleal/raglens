...
export function PetFeederSettings() {
  ...
  return (
    <>
      ...
      {showNtfyHelp && <NtfyHelpModal onClose={() => setShowNtfyHelp(false)} />}
    </>
  );
}

function NtfyHelpModal({ onClose }: { onClose: () => void }) {
  // Stacked above the AddonSettingsModal (z-index 110 > base 100) so
  // the click-outside-to-close picks this one first when the user
  // clicks outside the help panel.
  return (
    <Modal title="Como configurar o ntfy" onClose={onClose} zIndex={110}>
      <section className="modal-section">
        <p>
          <strong>ntfy</strong> é um serviço gratuito de notificações
          por push. O Raglens manda uma mensagem para um{" "}
        ...
        <ol className="modal-numbered">
          ...
          <li>
            ...
            escolha um <strong>nome único</strong> para o seu tópico.
            Qualquer pessoa que souber o nome do tópico recebe as
            suas notificações, então trate o nome como uma senha
            curta (ex: <code>pet-abelha-rainha-x7k2</code>) — evite nomes
            óbvios como <code>raglens</code> ou <code>pet</code>.
          </li>
          <li>
            Cole o mesmo nome do tópico no campo{" "}
            <strong>“Tópico ntfy”</strong> aqui no Raglens.
          </li>
          <li>
            Marque <strong>“Ativar notificações por push”</strong>.
            Pronto — o Raglens vai mandar uma notificação cada vez
            que o mascote entrar na faixa ideal ou na zona de
            perigo.
          </li>
        </ol>
        <p className="muted modal-hint">
